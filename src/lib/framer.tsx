    ...
    >
      {tabs.map((item, i) => {
        ...
        return (
          <button
            key={i} 
            className={`text-lg relative flex items-center h-10 px-4 z-20 bg-transparent  cursor-pointer select-none transition-colors w-full justify-center hover:bg-white/10 duration-200 py-8 ${!isActive ? 'text-white/60' : 'text-white/90 font-bold'}`}
